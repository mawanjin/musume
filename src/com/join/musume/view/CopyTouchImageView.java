 ...
 * Time: 5:13 PM
 */
public class CopyTouchImageView extends ImageView {

    String TAG = CopyTouchImageView.class.getName();
    Canvas _canvas;
    float x_down = 0;
    float y_down = 0;
    ...
    public CopyTouchImageView(TouchImageViewActivity activity) {
        super(activity);
    ...
    protected void onDraw(Canvas canvas) {
        _canvas = canvas;
        canvas.save();
        canvas.scale(postScale1,postScale2,postScale3,postScale4);
        canvas.drawBitmap(gintama, matrix, null);
        canvas.restore();
        if(drawOthers){
            canvas.save();
            canvas.drawBitmap(testBitMap,10,10,null);
            canvas.restore();
        }
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction() & MotionEvent.ACTION_MASK) {
            case MotionEvent.ACTION_DOWN:
                ...
                x_down = event.getX();
                y_down = event.getY();
                Log.i(TAG,"x_down="+x_down+";y="+y_down);
